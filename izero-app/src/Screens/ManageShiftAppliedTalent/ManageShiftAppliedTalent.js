...
export default function ManageShiftAppliedTalent({navigation, route}) {
  let data = route?.params;
  let jobName = data?.jobName;
  let jobAddress = data?.jobAddress;
  let avatar = data?.avatar;
  //let jobShiftsParams = data?.jobShifts;

  const [vissible, setVissible] = useState(true);
  const [loading, setLoading] = useState(true);

  // const [jobShifts, setjobShifts] = useState(jobShiftsParams);
  ...
  useEffect(() => {
    console.log('ManageShiftAppliedTalent'), [];
  });
