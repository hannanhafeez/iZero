...
class SelectShiftsClient extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  getJobDetails = async () => {
    ...
    try {
      let res = await Api.get(
        `/jobshiftdetail?id=${this.props.route.params.jobIDParams}`,
        {
          headers: {
      ...
      console.log('Job details for Manage job Shifts API res', res);
  ...
  favAdd = async () => {
    ...
    try {
      ...
      data.append('job_id', this.props.route.params.jobIDParams);
  ...
  manageShiftsForJob = async () => {
    ...
    if (this.state.jobShiftIdsArr.length < 1) {
      ...
      try {
        ...
        Alert.alert('', 'Staff has successfully been offered this shift', [
          {
            text: 'OK',
            onPress: () =>
              this.props.navigation.navigate('ManageShiftAppliedClient', {
      ...
      <Container>
        <View
          ...
          <View
            ...
            {this.state.checkJobDetails ? (
              <Text
                style={[styles.address, {color: colors.darkGreyLow}]}></Text>
            ) : (
              <Text
                style={[styles.address, {color: colors.darkGreyLow}]}></Text>
            )}
                ...
                return (
                  <>
                    <ShiftCard
                      ...
                      txtStyle={item.isSelect ? null : {color: colors.darkBlue}}
                      text={
                        // this.state.applliedAlready.includes(item.data)
                        //   ? 'Already Applied':
                        item.isSelect ? 'Booked' : 'Book'
                      }
                      startDate={item.data}
        ...
        {this.state.checkButtonShifts == true ? (
          <View style={styles.sticky}>
            <Button
              style={styles.Button}
              onPress={() => this.manageShiftsForJob()}>
...
export default SelectShiftsClient;
