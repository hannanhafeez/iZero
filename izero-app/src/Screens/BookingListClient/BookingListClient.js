...
export default function BookingListClient({navigation, route}) {
  ...
  const [temp1, setTemp1] = useState([
    {
      avatar: 'http://obstechnologia.com/izero/avatar.png',
  ...
  useEffect(() => {
    console.log('BookingListClient');
    getSearchData();
    setCheckSearch(false);
    //getClientClanderList();
    getCalenderAdded();
  }, [isFocused]);
  ...
  const getClientClanderList = async () => {
    setLoading(false);
    let temp = [];
    try {
      let res = await Api.get('/client_calender_list', {
        headers: {Authorization: `Bearer ${jwt}`, Accept: 'application/json'},
      });
      console.log('Calendar List Api Response', res?.data?.data);
  ...
  const getCalenderAdded = async () => {
    setLoading(false);
    try {
      let res = await Api.get(`/client_calender_list?date=${bookListParams}`, {
        headers: {Authorization: `Bearer ${jwt}`, Accept: 'application/json'},
      });
                  ...
                  renderItem={({item, index}) => {
                    ...
                    return (
                      <>
                        <TouchableOpacity
                          onPress={() =>
                            navigation.navigate('BookingAddCalenderClient', {
                              jobData: jobData,
                            })
                          }>
